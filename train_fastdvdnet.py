...
# QAT: helpers for the fuse -> prepare_qat lifecycle.
...
def main(**args):
    ...
    model = FastDVDnet(
        ...
        train_mode=True,
        quantize_bank=not args['no_quantize_bank'],   # QAT: bank INT8 by default
...
if __name__ == "__main__":
    ...
    # QAT: quantization-aware training flags.
    parser.add_argument("--qat", action='store_true',
                        help="QAT: enable quantization-aware training")
    parser.add_argument("--no_quantize_bank", action='store_true',
                        help="QAT: train attention/bank in float (compare only; deploy uses INT8 bank)")
    parser.add_argument("--input_bits", type=int, default=8,
                        help="QAT: deploy-time input bit-depth (8 now; 10/16 later)")
