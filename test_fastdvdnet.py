...
Flow (both modes): clean RGB -> add noise (RGB) -> [YUV path: RGB->YUV420 ->
model -> residuals -> denoised YUV -> RGB] / [RGB path: model -> residual ->
denoised RGB] -> RGB PSNR. Select YUV with --yuv.
"""
...
def save_frames(frames, save_dir, prefix, tag):
...
def write_psnr_table(path, rows, avg_noisy, avg_den):
    ...
    lines += [sep,
              '{:<{w}}  {:>7}  {:>11.4f}  {:>11.4f}  {:>+10.4f}'.format(
                  'AVERAGE', '-', avg_noisy, avg_den, avg_den - avg_noisy, w=col_w),
              sep]
...
def test_fastdvdnet(**args):
    ...
    logger.info("FastDVDnet Test (%s)" % ("YUV420" if yuv else "RGB"))
    logger.info(f"  model      : {args['model_file']}")
    logger.info(f"  noise_sigma: {sigma_int}/255 = {args['noise_sigma']:.5f}")
    logger.info(f"  lam        : {lam_int}/255 = {args['lam']:.5f}")
    ...
    psnr_all, psnr_noisy_all, table_rows = [], [], []
    ...
    with torch.no_grad():
        for seq_idx, seq_dir in enumerate(seq_dirs, 1):
            ...
            seq = torch.from_numpy(seq)        # CPU, (T,3,H,W) clean RGB [0,1]
            load_t = time.time() - seq_start

            seq_seed = args['seed'] + abs(hash(seq_name)) % (2 ** 16)
            torch.manual_seed(seq_seed)

            # Noise in RGB (shot then read) — same order as training
            ...
            # denoise_seq returns denoised RGB in both modes
            ...
            psnr_all.append(psnr); psnr_noisy_all.append(psnr_noisy)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test FastDVDnet (RGB/YUV)")
    ...
    print("\n### Testing FastDVDnet (%s) ###" % ("YUV420" if argspar.yuv else "RGB"))
