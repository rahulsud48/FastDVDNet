...
def svd_orthogonalization(lyr):
    # Only process actual Conv2d layers
    ...
    # Do not orthogonalize 1x1 convs if you want behavior close to original FastDVDnet
    # because original orthogonalization was mainly intended for spatial convs.
    if f1 == 1 and f2 == 1:
        return
    ...
    try:
        u, _, v = torch.linalg.svd(weights, full_matrices=False)
        weights = torch.mm(u, v)
        weights = weights.view(f1, f2, c_in, c_out).permute(3, 2, 0, 1).contiguous()
        lyr.weight.data.copy_(weights)
    except RuntimeError:
        pass
