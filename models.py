"""
FastDVDnet — Blind single-frame RGB input with KV Bank cross-attention.

Changes:
  - Blind denoiser: noise_map removed from input entirely.
    Model takes only (N, 3, H, W) RGB frame — no sigma conditioning.
    InputCvBlock now accepts 3 channels instead of 4.
  - forward() still returns (predicted_residual, denoised) for residual loss.
"""
...
class InputCvBlock(nn.Module):
    """First encoder block — blind: RGB only (N, 3, H, W), no noise map."""
    def __init__(self, out_ch):
        super(InputCvBlock, self).__init__()
        self.interm_ch = 30
        # 3 channels only — no noise map concatenated
        self.convblock = nn.Sequential(
            nn.Conv2d(3, self.interm_ch, kernel_size=3, padding=1, bias=False),
...
class BottleneckCrossAttn(nn.Module):
    """Cross-attention at bottleneck: Q=current frame, KV=past frames from bank."""
...
class DenBlock(nn.Module):
    """
    Blind FastDVDnet denoising block.
    Input : (N, 3, H, W) noisy RGB — no noise map.
    Output: (predicted_residual, denoised) both (N, 3, H, W).
    """
    def __init__(self, bank_size: int = 10, num_heads: int = 4, pool_size: int = 8):
        ...
        self.inc    = InputCvBlock(out_ch=self.chs_lyr0)  # 3ch input
    ...
    def forward(self, frame_t: torch.Tensor, bank: KVBank):
        """
        Args:
            frame_t : (N, 3, H, W)  noisy RGB in [0,1] — no noise map
            bank    : KVBank
        Returns:
            predicted_residual : (N, 3, H, W)
            denoised           : (N, 3, H, W)
        """
        x0 = self.inc(frame_t)           # 3ch input directly
        x1 = self.downc0(x0)
        x2 = self.downc1(x1)
        ...
        x2 = self.upc2(x2)
        x1 = self.upc1(x1 + x2)
        predicted_residual = self.outc(x0 + x1)
...
class FastDVDnet(nn.Module):
    """Blind FastDVDnet: single RGB frame + KV bank, no noise map input."""
    def __init__(self, bank_size: int = 10, num_heads: int = 4, pool_size: int = 8):
    ...
    def forward(self, frame_t: torch.Tensor, bank: KVBank):
        """
        Args:
            frame_t : (N, 3, H, W)  — no noise_map argument
            bank    : KVBank
        """
        return self.temp(frame_t, bank)


if __name__ == "__main__":
    bank_size = 10
    model = FastDVDnet(bank_size=bank_size)
    bank  = KVBank(bank_size=bank_size)
    model.eval()
    with torch.no_grad():
        for t in range(5):
            frame = torch.rand(1, 3, 96, 96)
            residual, denoised = model(frame, bank)
